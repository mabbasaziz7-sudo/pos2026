...
// true بشكل افتراضي إذا لم يُعيَّن
...
export function printOfferCard(offer: Offer, settings: Settings | null) {
  ...
  const showLogo   = on(settings?.offerCardShowLogo);
  const showTarget = on(settings?.offerCardShowTarget);
  const showDates  = on(settings?.offerCardShowDates);
  const showStatus = on(settings?.offerCardShowStatus);

  const logo = showLogo && settings?.storeLogo
    ? `<img src="${settings.storeLogo}" style="height:48px;object-fit:contain;display:block;margin:0 auto 8px;">`
    : '';
  ...
  const target = offer.targetType === 'category' ? `على فئة: ${offer.category}`
    : offer.targetType === 'bundle' ? 'عرض مجمّع' : 'على منتجات محددة';

  openCardWindow(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>بطاقة عرض</title>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    body { font-family:${font}; background:#f0f0f0; display:flex; justify-content:center; padding:20px; }
    .card { width:380px; background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,.15); }
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      ${showTarget ? `<div class="target">${target}</div>` : ''}
      ${showDates ? `<div class="dates">
      ...
      ${showStatus ? `<span class="status ${offer.isActive ? 'active' : 'inactive'}">${offer.isActive ? 'ساري' : 'منتهي'}</span>` : ''}
    </div>
  </div>
...
export function printCouponCard(coupon: Coupon, settings: Settings | null) {
  ...
  const showLogo       = on(settings?.couponCardShowLogo);
  const showBarcode    = on(settings?.couponCardShowBarcode);
  const showExpiry     = on(settings?.couponCardShowExpiry);
  const showMinPurch   = on(settings?.couponCardShowMinPurchase);
  const showUsageCount = on(settings?.couponCardShowUsageCount);

  const logo = showLogo && settings?.storeLogo
    ? `<img src="${settings.storeLogo}" style="height:40px;object-fit:contain;display:block;margin:0 auto 6px;">`
    : '';
  const discount = coupon.discountType === 'percentage' ? `${coupon.discountValue}% خصم` : `${coupon.discountValue.toFixed(3)} خصم`;

  openCardWindow(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>كوبون خصم</title>
  ...
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .store-line { display:flex; align-items:center; gap:8px; margin-bottom:10px; font-weight:bold; font-size:14px; color:${accent}; }
  ...
  <div class="coupon">
    <div class="coupon-top">
      <div class="store-line">${logo}${storeName}</div>
      <div class="coupon-badge">${discount}</div>
      <div class="coupon-label">🎫 كوبون خصم</div>
    </div>
    <div class="coupon-bottom">
      ${showMinPurch ? `<div class="row"><span>الحد الأدنى للشراء</span><span style="font-weight:bold;">${coupon.minPurchase > 0 ? coupon.minPurchase.toFixed(3) : 'لا يوجد'}</span></div>` : ''}
      ${showUsageCount ? `<div class="row"><span>الاستخدامات المتبقية</span><span style="font-weight:bold;color:${accent};">${coupon.maxUses - coupon.usedCount} من ${coupon.maxUses}</span></div>` : ''}
      ${showExpiry ? `<div class="row"><span>صالح حتى</span><span style="font-weight:bold;color:#e11d48;">${new Date(coupon.expiryDate).toLocaleDateString('ar-SA')}</span></div>` : ''}
      ${showBarcode ? `<div class="code-area">
        <svg id="barcode" style="display:block;margin:8px auto;max-width:100%;"></svg>
        <div class="code-text">${coupon.code}</div>
      </div>` : `<div class="code-area" style="margin-top:8px;"><div class="code-text" style="font-size:16px;">${coupon.code}</div></div>`}
    </div>
  </div>
  <script>
    ${showBarcode ? `JsBarcode('#barcode','${coupon.code}',{format:'CODE128',width:1.8,height:48,displayValue:false,margin:4});` : ''}
    window.print();
  </script></body></html>`);
}
...
export function printVoucherCard(voucher: Voucher, settings: Settings | null) {
  ...
  const showLogo    = on(settings?.voucherCardShowLogo);
  const showBarcode = on(settings?.voucherCardShowBarcode);
  const showBalance = on(settings?.voucherCardShowBalance);
  const showStatus  = on(settings?.voucherCardShowStatus);

  const logo = showLogo && settings?.storeLogo
    ? `<img src="${settings.storeLogo}" style="height:42px;object-fit:contain;display:block;margin:0 auto 8px;">`
    : '';

  openCardWindow(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>قسيمة هدايا</title>
  <script src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.5/dist/JsBarcode.all.min.js"></script>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    body { font-family:${font}; background:#f0f0f0; display:flex; justify-content:center; padding:20px; }
  ...
  <div class="voucher">
    ...
    <div class="voucher-body">
      ...
      ${showBalance ? `<div class="balance-section">
      ...
      <div class="code-section">
        ${showBarcode ? `<svg id="barcode" style="display:block;margin:0 auto;max-width:100%;"></svg>` : ''}
        <div class="code-text">${voucher.code}</div>
      </div>
      ${showStatus ? `<span class="status-badge ${voucher.isActive && voucher.balance > 0 ? 'active' : 'inactive'}">
        ${voucher.isActive && voucher.balance > 0 ? 'نشطة' : !voucher.isActive ? 'معطّلة' : 'مستهلكة'}
      </span>` : ''}
    </div>
  </div>
  <script>
    ${showBarcode ? `JsBarcode('#barcode','${voucher.code}',{format:'CODE128',width:1.8,height:48,displayValue:false,margin:4});` : ''}
    window.print();
  </script></body></html>`);
}
...
export function printLoyaltyCard(customer: Customer, settings: Settings | null) {
  ...
  const showLogo    = on(settings?.loyaltyCardShowLogo);
  const showBarcode = on(settings?.loyaltyCardShowBarcode);
  const showBalance = on(settings?.loyaltyCardShowBalance);
  const showPhone   = on(settings?.loyaltyCardShowPhone);
  ...
  const logo = showLogo && settings?.storeLogo
    ? `<img src="${settings.storeLogo}" style="height:44px;object-fit:contain;">`
    : (showLogo ? `<div style="font-size:20px;font-weight:bold;">${storeName.slice(0,2)}</div>` : '');

  openCardWindow(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>بطاقة ولاء</title>
  <script src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.5/dist/JsBarcode.all.min.js"></script>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .points-section .points-label { font-size:9px; opacity:.8; }
    .points-section .points-value { font-size:24px; font-weight:bold; line-height:1; }
    .points-section .points-sub { font-size:9px; opacity:.8; }
  ...
  <div class="loyalty-card">
    ...
    ${showBalance ? `<div class="card-bottom">
  ...
  <div class="card-back">
    ...
    <div class="card-back-body">
      ...
      <div class="balance-note">
        ${showBalance ? `رصيد الولاء: <strong style="color:${accent};">${customer.loyaltyPoints} نقطة</strong> = <strong>${pointsValue}</strong><br>` : ''}
        ${showPhone && customer.phone ? `📞 ${customer.phone}` : ''}
      </div>
    </div>
  </div>
  <script>
    ${showBarcode ? `JsBarcode('#barcode','${memberNo}',{format:'CODE128',width:1.8,height:40,displayValue:false,margin:4});` : ''}
    window.print();
  </script></body></html>`);
}

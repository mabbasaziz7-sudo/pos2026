...
import { formatCurrency, formatDate, formatNumber } from '@/lib/store';
import {
  BarChart3,
  TrendingUp,
  TrendingDown,
            ...
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-full h-2.5">
                <div
                  ...
                  style={{ width: `${totalSales ? (totalCash / totalSales) * 100 : 0}%` }}
                />
              </div>
            </div>
            <div>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {topProducts.map((product, idx) => (
              <div key={idx} className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
                ...
                <span className="text-sm font-medium text-emerald-600">{formatCurrency(product.revenue)}</span>
              </div>
            ))}
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-emerald-600">{formatCurrency(sale.paid)}</td>
